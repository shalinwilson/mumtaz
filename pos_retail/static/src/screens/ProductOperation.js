...
odoo.define('pos_retail.screen_products_operation', function (require) {
    ...
    var ButtonProductsOperationScreen = screens.ActionButtonWidget.extend({
        template: 'ButtonProductsOperationScreen',
        init: function (parent, options) {
            var self = this;
            this._super(parent, options);
            this.pos.bind('open:products-operation-screen', function () {
                self.button_click();
            });
        },
        button_click: function () {
            this.pos.gui.show_screen('products_operation');
        },
        get_count_products: function () {
            return this.pos.db.product_ids.length
        }
    });

    screens.define_action_button({
        'name': 'ButtonProductsOperationScreen',
        'widget': ButtonProductsOperationScreen,
        'condition': function () {
    ...
    var popup_create_pos_category = PopupWidget.extend({
        ...
        show: function (options) {
            ...
            this._super(options);
            var contents = this.$('.create_category');
            ...
            this.$('.confirm').click(function () {
                ...
                if (self.uploaded_picture) {
                    fields.image_128 = self.uploaded_picture.split(',')[1];
                }
                if (fields['parent_id']) {
    ...
    var popup_create_product = PopupWidget.extend({
        ...
        click_confirm: function () {
            ...
            if (this.uploaded_picture) {
                fields.image_1920 = this.uploaded_picture.split(',')[1];
            }
                    ...
                    if (product && product['barcode']) {
                        ...
                        if (!self.pos.proxy.printer) {
                            return self.pos.gui.show_popup('confirm', {
                                title: _t('Warning'),
                                body: _t('Your printer turn off, Please setup posbox ip address and printer')
                            })
                        }
                        if (self.pos.proxy.printer) {
                            self.pos.proxy.printer.print_receipt(product_label_html);
                            return self.pos.gui.show_popup('dialog', {
                                title: _t('Alert'),
                                body: _t('Print Barcode succeed'),
                                color: 'success'
                            })
                        }
                    } else {
