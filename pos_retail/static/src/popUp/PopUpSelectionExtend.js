    ...
    var popup_selection_extend = PopupWidget.extend({
        ...
        show: function (options) {
            ...
            for (var i=0; i < this.sub_datas.length; i++) {
                var data = this.sub_datas[i];
                if (data.selected) {
                    this.selected_ids.push(data.id)
                }
            }
            ...
            this._super(options);
            if (options.header_button) {
                var header_button = document.createElement('span');
                header_button.innerHTML = options.header_button;
                header_button.addEventListener('click', options.header_button_action);
                this.el.querySelector('.header_button').appendChild(header_button);
            }
            ...
            if (!this.mutli_choice) {
                ...
                this.$('.line-select').click(function () {
                    ...
                    if ($(this).closest('.line-select').hasClass("item-selected") == true) {
                        ...
                        var record = _.find(self.sub_datas, function (record) {
                            return record.id == selected_id
                        })
        ...
        unselect_items: function () {
            for (var i = 0; i < this.sub_datas.length; i++) {
                this.sub_datas[i].selected = false
            }
        },
